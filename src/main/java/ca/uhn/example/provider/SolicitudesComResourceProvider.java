 ...
 * @author borisgr04
 */
public class SolicitudesComResourceProvider implements IResourceProvider{
    private final Map<Long, Deque<SolicitudCommunication>> myComm = new HashMap<Long, Deque<SolicitudCommunication>>();
    private Long myNextId = 1L;
    
     /**
     * Constructor
     */
    public SolicitudesComResourceProvider() {
        SolicitudCommunication c1 = new SolicitudCommunication();
        ...
        LinkedList<SolicitudCommunication> list = new LinkedList<SolicitudCommunication>();   
        list.add(c1);
    ...
    public Class<? extends IResource> getResourceType() {
        return SolicitudCommunication.class;
    }
     ...
     */
    @Read()
    public SolicitudCommunication read(@IdParam IdDt theId) {
       /* Communication retVal = myComm.get(theId.getIdPartAsLong());
        ...
        return retVal;
        */
        Deque<SolicitudCommunication> retVal;
	try {
		...
		if (theId.hasVersionIdPart() == false) {
			...
			for (SolicitudCommunication nextVersion : retVal) {
     ...
     */
    @Create
    public MethodOutcome create(@ResourceParam SolicitudCommunication theComm) {
        long id = myNextId++;
     ...
     */
    @Search
    public List<SolicitudCommunication> search() {
       LinkedList<SolicitudCommunication> retVal = new LinkedList<SolicitudCommunication>();
        for (Deque<SolicitudCommunication> nextList : myComm.values()) {
			SolicitudCommunication next = nextList.getLast();
			retVal.add(next);
	}
	return retVal;
    }
    ...
    @Update()
    public MethodOutcome update(@IdParam IdDt theId, @ResourceParam SolicitudCommunication theCom) {
    ...
    private void addNewVersion(SolicitudCommunication theComm, Long theId) {
		InstantDt publishedDate;
		if (!myComm.containsKey(theId)) {
			myComm.put(theId, new LinkedList<SolicitudCommunication>());
			publishedDate = InstantDt.withCurrentTime();
		} else {
			SolicitudCommunication currentPatitne = myComm.get(theId).getLast();
		...
		Deque<SolicitudCommunication> existingVersions = myComm.get(theId);
