...
public class CommunicationResourceProvider implements IResourceProvider {
    ...
    private final Map<Long, Deque<Communication>> myComm = new HashMap<Long, Deque<Communication>>();
     ...
     */
    @Read()
    public Communication read(@IdParam IdDt theId) {
	...
	try {
		...
		if (theId.hasVersionIdPart() == false) {
			...
			for (Communication nextVersion : retVal) {
     ...
     */
    @Create
    public MethodOutcome create(@ResourceParam Communication theComm) {
        long id = myNextId++;
    ...
    public List<Communication> search() {
       LinkedList<Communication> retVal = new LinkedList<Communication>();
        for (Deque<Communication> nextList : myComm.values()) {
			Communication next = nextList.getLast();
			retVal.add(next);
	}
	return retVal;
    }
    ...
    @Search
    public List<Communication> searchSolicitudesPacient(
            @RequiredParam(name=Communication.SP_PATIENT ) String theIdPaciente,
            @RequiredParam(name=Communication.SP_CATEGORY) String theCategoria
            ) {
        ProcessorComm pc = new ProcessorComm();
        return pc.ObtenerSolicitudes (theIdPaciente.toString());
    }
    
    /*  @Search
    public List<Communication> searchSolicitud(
    @RequiredParam(name=Patient.SP_IDENTIFIER) StringParam theParameter) {
       
        //
        Invocar al Procesador de Comunicacione y Retornar la Lsista
        //
    }*/
    
    @Update()
    public MethodOutcome update(@IdParam IdDt theId, @ResourceParam Communication theCom) {
    ...
    private void addNewVersion(Communication theComm, Long theId) {
		InstantDt publishedDate;
		if (!myComm.containsKey(theId)) {
			myComm.put(theId, new LinkedList<Communication>());
			publishedDate = InstantDt.withCurrentTime();
		} else {
			Communication currentPatitne = myComm.get(theId).getLast();
		...
		Deque<Communication> existingVersions = myComm.get(theId);
